  ...
  video.addEventListener(
    ...
    function (ev) {
      if (!streaming) {
        ...
        videoWidth = Math.min((6 * window.innerWidth) / 7, 640);
        videoHeight = 480;
...
function processVideo() {
  ...
  if (detectFace.checked) {
    ...
    for (let i = 0; i < faceVect.size(); i++) {
      ...
      if (detectEye.checked) {
        ...
        eyeClassifier.detectMultiScale(faceMat, eyeVect);
  ...
  // fix_dpi();
  predictMask(faces, canvasInputCtx, size, canvasOutputCtx);
...
function opencvIsReady() {
  ...
  startCamera();
  startPrediction();
  text.innerHTML = "Ready to recognise your face";
}
